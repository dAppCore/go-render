...
A leading space-only gutter -- indenting a line with blank columns alone -- is not expressible in terminal output, by design at two layers. At parse, a `CharData` run that is entirely whitespace is dropped (S:S2) so source indentation between elements disappears; a standalone gutter run before a child element goes with it. At terminal render, the block assembler trims each inline paragraph's leading and trailing edge (`blocks` runs `strings.TrimSpace` on every flushed run) so a paragraph never carries stray indentation, which removes a leading gutter that *did* survive parse by being glued to its text (`  text`). A whitespace entity does not slip past either layer: `&#32;`, `&#160;` (NBSP) and `&#8199;` (figure space) all decode to whitespace *before* either sees them, and every Unicode space character is `unicode.IsSpace`, so `strings.TrimSpace` removes it the same as a plain space; `<raw>  text</raw>` preserves the spaces through parse, but the paragraph flush still trims them at the edge. The blessed idiom for a visible leading gutter is therefore a **marker glyph** -- a non-space leading character the flush keeps, `○ ` or `· ` before the text, exactly the tab-strip shape S:S8.4 already uses -- or **structural padding**: the C content gutter (S:S15.2), or a themed slot's own padding (`Sidebar`, `Card`). Reach for a marker glyph or a padded region, never a run of spaces.
