...
### 8.7 Formatting pipes: `{{ path | pipe }}`

A `{{path}}` token -- in a text run (S:S8.3) or a `<verbatim value="{{path}}"/>` bind (S:S6.5) -- may carry one trailing formatting pipe: `{{ path | pipe }}` runs the resolved value through a named formatter, and `{{ path | pipe:arg }}` passes it one further argument (`{{ minutesAgo | ago:minutes }}`). Whitespace around `|` and `:` is tolerated -- `{{ x | ago : minutes }}` and `{{x|ago:minutes}}` are the same token. There is no chaining (`{{ x | a | b }}` is not a v1 construct) and no user-defined pipes: the vocabulary is closed, matching S:S1's "no expression language" rule -- a pipe is one more named lookup, not an operator.

**The builtin pipe vocabulary is closed and validated at parse time:** `number`, `decimal`, `percent`, `ordinal`, `ago`, `size`, `bytes`. An unrecognised name -- `{{ x | bogus }}` -- is a loud, position-accurate parse error (S:S9) naming the bad pipe, never a silent fallback to literal text. This is a deliberate asymmetry with S:S8.4's leniency for a malformed *path*: a `"|"` inside `{{...}}` has no other meaning in CTML, so once one appears the token is unambiguously a pipe attempt, and a broken attempt is a document defect worth surfacing rather than papering over. The leniency for a malformed *path half* is unchanged -- `{{not a path | number}}` still falls through to literal text exactly as an unpiped near-miss does, because the whole token is not recognised as a bind at all.

**Legal contexts are text runs and verbatim value binds only.** A pipe inside an attribute value's `{{path}}` interpolation (S:S5) -- `class="{{ x | number }}"` -- is a parse error, whether or not the pipe name is recognised: an attribute is code (a class name, an id, an href), not copy, and a formatted display string has no business there. `args="..."` tokens (S:S6.4) do not recognise pipe syntax either -- a `{{x|number}}` args token is simply not a valid `{{path}}` reference (the `"|"` is not a path character), so it falls through to S:S8.4's existing literal-argument handling, unchanged from before pipes existed.

**Resolution happens at construction time, via `dappco.re/go/html`'s exported `FormatValue`, not through a render-time `Context`.** Every other `{{path}}` value bind already resolves when the node is built -- once at document scope inside `Parse`, once per row when `<each>` builds that row's node (S:S1.3, S:S8.1) -- and a pipe extends that same token, so it resolves on exactly the same schedule: `html.FormatValue(pipe, value, arg)` runs immediately, and the *formatted string* is what gets baked into the constructed `Text`/`Verbatim` node. This is why a piped bind composes for free with terminal width measurement (S:S15.1): by the time `FitSlots` measures a slot's rendered content, the string it measures already is the post-format string, with no special-casing needed anywhere in the terminal renderer. It is also why pipes do **not** go through `go-html`'s `Context.Formatter` seam the way `Text`'s i18n key lookup goes through `Context.Translator` at render time (`ctx.service.T(key, args...)`, resolved fresh on every `Render` call): `ctml.Parse` has no `*Context` to route through -- a `Context` only exists later, when a caller renders the already-built tree -- so there would be nothing to plug a per-render `Context.Formatter` into. `Context.Formatter` still exists in `go-html` (mirroring `Translator`'s shape: an interface, a `Context` field, `NewContextWithFormatter`/`SetFormatter`) for a hand-built tree that wants render-time-pluggable formatting of its own; `html.FormatValue` is both the entry point ctml calls directly and the default that seam falls back to when no explicit `Formatter` is installed, so the two paths format identically unless a caller deliberately overrides one.

**A resolver miss renders empty, never a formatted zero.** `{{ missing | number }}` follows the same miss-is-empty rule as an unpiped bind (S:S8.3): a field that is not present in the active row or `Values` renders as empty text (or an empty `Verbatim`), because a missing value is data absence, not the number zero -- piping it through a formatter must not manufacture a value that was never there.
...
A page may still want a *different fixed width* for a side slot -- a wider inspector pane, say -- without giving up the fixed side-by-side geometry. `TermOptions.SidebarWidth` and `TermOptions.AsideWidth` are that request: a positive value overrides the 24/28 default for L or R in the wide band (`AsideWidth: 32` renders R 32 columns wide), and C absorbs the difference. This is a width *request*, not a width *accessor* -- S:S15.5 keeps the fixed budgets unexported and reads a rendered slot's *actual* width back from the box map; this states the budget the caller *wants* going in, which the renderer sizes to. Width is layout, not paint, so the request rides `TermOptions` alongside `Width` and `FitSlots`, never the theme -- the theme measures chrome *from* its styles (S:S15.5) but carries no column budget. A zero or negative value keeps the fixed default, so an omitted option is byte-identical to before the fields existed. The override touches only the wide side-by-side band: below the 80-column stack threshold both slots stack at full width, and under FitSlots each is content-sized, so neither path reads the budget. A request so wide it would starve C -- L + R + the two gutters leaving less than the minimum for C -- floors C at the minimum content width and lets the frame exceed its nominal width, the same caller-owns-content boundary FitSlots draws for over-wide content.
...
### 15.7 Mid-page vertical pane pairs: nest a layout, don't add a band

A shape that looks unsupported: two independently-sized panes stacked *vertically* mid-page with a rule between -- an "overlay with the inspector open". HLCRF seems to have no such pair: H and F are whole-page bands, and the automatic L/C/R *vertical* stacking engages only below 80 columns, so at a mid-page width there is apparently no vertical split to reach for.

There is, and it needs no new construct -- **the frame's own band stacking is a vertical stack.** Only the *middle band's* L/C/R packing is width-gated (side by side at `>= 80`, stacked below); the H / middle / F bands themselves stack vertically at **any** width, each sized to its own content, top to bottom. So a vertical pane pair is a layout that uses two adjacent bands, and the divider between them is the border the theme already gives one of them: an **`HC`** layout renders H (upper pane) over C (lower pane) with H's bottom border as the rule, a **`CF`** layout renders C over F with F's top border as the rule, and an **`HCF`** renders three panes with a divider above and below the middle. `NewLayout("HC").H(top).C(bottom)` *is* the pair; the bands carry the default header/content styling, which a theme restyles when neither pane is chrome.

**Placing it mid-page is nesting, which already works.** A `Layout` is a `Node`, so the pair-layout drops straight into an outer page's slot and renders width-bounded within that column -- the same clone-on-render nesting the HTML compositor performs (S:S14; `TestTermLayout_RenderTerm_Nested` and `TestTermLayout_VerticalPanePair` pin it). `NewLayout("HCF").H(pageHead).C(paneLayout).F(pageFoot)` gives the page its own header and footer with the two-pane split filling the main area between them, at a mid-page width, no width-gating involved. So the shape is a **documented idiom, not a missing feature** -- HLCRF's vocabulary already expresses it by composition.

**What stays host-composed is interaction state, and that is correct, not a gap.** If the two panes each need their *own scroll offset and focus* -- a `bubbletea` app where the main pane and the inspector scroll independently -- that state does not belong to go-html. The terminal renderer is a **one-pass, stateless tree -> string** function: a `Layout` carries no terminal state (it is also the WASM-linked HTML compositor, S:S15.1), and a render reads the tree and returns a string with no mutable per-pane offset anywhere in it. A renderer that owned scroll positions would have to own an event loop and per-frame mutable state -- exactly what the interaction layer already owns. The right architecture is unchanged: the host runs two `Bubbles` viewports, renders each pane's *content* through go-html to a string, and lets each viewport own its own scroll. go-html composes the document; the event loop composes the interaction -- the same separation that keeps `FitSlots` and mouse boxes render-time concerns and leaves the `tea.MouseMsg` type to the caller (S:S14). The vertical-pair *shape* is the nested idiom above; independent *scroll* is the host's.
