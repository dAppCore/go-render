...
func TestParseTape_Good(t *testing.T) {
	tests := []struct {
		...
		{
			name: "good: Expect NotText takes a quoted substring",
			src:  `Expect NotText "goodbye"`,
			want: []command{{Verb: "Expect", Args: []string{"NotText", "goodbye"}, Line: 1}},
		},
		{
			name: "good: Expect Line takes a line number and quoted text",
			src:  `Expect Line 0 "Welcome"`,
			want: []command{{Verb: "Expect", Args: []string{"Line", "0", "Welcome"}, Line: 1}},
		},
		{
			name: "good: Expect Width takes a display-cell count",
			src:  "Expect Width 40",
			want: []command{{Verb: "Expect", Args: []string{"Width", "40"}, Line: 1}},
		},
		{
		...
		{
			name: "good: Click takes one box id argument",
			src:  "Click banner",
...
func TestParseTape_Bad(t *testing.T) {
	tests := []struct {
		...
		{"bad: Expect NotText with no substring", "Expect NotText", 1},
		{"bad: Expect NotText with two arguments", "Expect NotText a b", 1},
		{"bad: Expect Line with no arguments", "Expect Line", 1},
		{"bad: Expect Line with only a line number", "Expect Line 0", 1},
		{"bad: Expect Line with a non-integer line number", "Expect Line first Welcome", 1},
		{"bad: Expect Line with a negative line number", "Expect Line -1 Welcome", 1},
		{"bad: Expect Line with an extra argument", "Expect Line 0 Welcome extra", 1},
		{"bad: Expect Width with no argument", "Expect Width", 1},
		{"bad: Expect Width with a non-integer value", "Expect Width wide", 1},
		{"bad: Expect Width with a negative value", "Expect Width -1", 1},
		{"bad: Expect Width with an extra argument", "Expect Width 40 extra", 1},
